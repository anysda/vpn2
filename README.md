...
### 0. (рекомендуется, **за 4 часа до деплоя**) — CAA-запись у регистратора домена

Если планируешь HTTPS-домен для панели (`vpn.example.com`) — добавь в DNS
**CAA-запись**, разрешающую выпуск сертификатов только Let's Encrypt
(тот, кого Caddy использует автоматически). Это блокирует выпуск чужих
сертификатов на твой домен — даже если CA скомпрометирован или кто-то
угнал DNS, никто кроме LE не сможет выдать валидный TLS для панели.

В панели управления домена добавь записи:
...
(`issuewild ";"` запрещает wildcard-сертификаты; `iodef` — куда CA
сообщит о попытке несанкционированного выпуска.)

DNS-кэш RIRов и публичных резолверов прогревается ≈4 часа; **поставь
CAA минимум за 4 часа до деплоя**, чтобы к моменту `./deploy.sh` Let's
Encrypt уже видел запись и сразу выпустил сертификат. Иначе первый
запуск Caddy упрётся в `CAA record prevents issuance` и придётся ждать.
